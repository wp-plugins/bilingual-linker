...
if ( ! class_exists( 'BL_Admin' ) ) {
		...
		function add_config_page() {
			...
			if ( function_exists( 'get_post_types' ) )
			{
				...
				foreach ( $post_types as $post_type )
				{
					...
					{
						add_meta_box ('bilinguallinker_meta_box', __('Bilingual Linker - Additional Post / Page Parameters', 'bilingual-linker'), array( $this, 'bl_postpage_edit_extra'), $post_type->name, 'normal', 'high');
					}
				}
			} 
			else
			{
				add_meta_box ('bilinguallinker_meta_box', __('Bilingual Linker - Additional Post / Page Parameters', 'bilingual-linker'), array( $this, 'bl_postpage_edit_extra'), 'post', 'normal', 'high');

				add_meta_box ('bilinguallinker_meta_box', __('Bilingual Linker - Additional Post / Page Parameters', 'bilingual-linker'), array( $this, 'bl_postpage_edit_extra'), 'page', 'normal', 'high');
			}
                ...
                <tr>
                    <td style='width: 200px'>
                        <?php 
                            ...
                            if ( empty( $langname) ) $langname = "Undefined Language";
                                echo $langname; ?> Link
                    </td>
                    <td>
 ...
 ?>
        ...
        function process_bl_options() {
            ...
            foreach ( array( 'numberoflanguages', 'language1name', 'language2name',
                            'language3name', 'language4name', 'language5name',
                            'language1defaulturl', 'language2defaulturl', 'language3defaulturl',
                            'language4defaulturl', 'language5defaulturl', 'language1beforelink',
                            'language2beforelink', 'language3beforelink', 'language4beforelink',
                            'language5beforelink', 'language1afterlink', 'language2afterlink',
                            'language3afterlink', 'language4afterlink', 'language5afterlink',
                            'language1linktext', 'language2linktext', 'language3linktext', 
                            'language4linktext', 'language5linktext') as $option_name ) {
...
function the_bilingual_link( $language_id = '', $post_id = '', $link_text = '', 
    ...
    if ( is_front_page() ) {
        ...
        $output = $code_before_link . "<a href='" . $final_default_url . "'>" . $final_link_text . "</a>" . $code_after_link;
 
        
    } elseif ( is_search() ) {
        
        $search_url = add_query_arg( 's', $_GET['s'], $final_default_url );
        $output = $code_before_link . "<a href='" . $search_url . "'>" . $final_link_text . "</a>" . $code_after_link;
    	...
    	if ($other_lang_url != '') {
            ...
            $output = $code_before_link . "<a href='" . $other_lang_url . "'>" . $final_link_text . "</a>" . $code_after_link;
        ...
        if ( is_year() ) {
            ...
            $archive_url = add_query_arg( array( 'year' => get_query_var( 'year' ), 'monthnum' => get_query_var( 'monthnum' ) ), $final_default_url ); 
        } elseif ( is_day() ) {
            $archive_url = add_query_arg( array( 'year' => get_query_var( 'year' ), 'monthnum' => get_query_var( 'monthnum' ), 'day' => get_query_var( 'day' ) ), $final_default_url ); 
        }
        
        $output = $code_before_link . "<a href='" . $archive_url . "'>" . $final_link_text . "</a>" . $code_after_link;
